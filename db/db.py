...
def delete_team(team_id):
    ...
    found = None
    for t in teams:
        if str(t.get("id")) == str(team_id) or t.get("name") == str(team_id):
            found = t
            break
    if not found:
        return False, f"Team with ID {team_id} not found"
    ...
    data["players"] = [p for p in data.get("players", []) if p.get("team") != team_name]
    data["matches"] = [m for m in data.get("matches", []) if m.get("teamA") != team_name and m.get("teamB") != team_name]
    data["standings"] = [s for s in data.get("standings", []) if s.get("team") != team_name]
    data["teams"] = [t for t in teams if str(t.get("id")) != str(team_id) and t.get("name") != team_name]
    set_collection("players", data["players"])
    set_collection("matches", data["matches"])
    set_collection("standings", data["standings"])
    set_collection("teams", data["teams"])
    recalculate_standings_internal()
